<script lang="ts">
  ...
  async function load() {
    ...
    seriesList = Object.entries(groups)
      .map(([name, comics]) => {
...
<main class="min-h-screen bg-gray-950 text-white p-6">
  <div class="max-w-7xl mx-auto">
    ...
    <div class="flex gap-4 mb-8 justify-center flex-wrap">
      ...
      <button
        on:click={() =>
          fetch("http://localhost:8000/scan", { method: "POST" }).then(load)}
    ...
    >
      {#each filteredSeries as serie}
        <div
          ...
          on:click={() => (selectedSeries = serie)}
        >
          <div
            class="bg-gray-900 rounded-xl overflow-hidden shadow-2xl hover:shadow-blue-500/50 transition"
          >
            {#if serie.cover}
              <img
                src={`http://localhost:8000/comics/${serie.cover.id}/page/0`}
                alt={serie.name}
                class="w-[100px] h-[100px] object-cover mx-auto group-hover:scale-105 transition"
              />
            {/if}
            <div class="p-4">
    ...
    {#if selectedSeries}
      ...
      >
        <div
          class="max-w-6xl max-h-full overflow-y-auto bg-gray-900 rounded-2xl p-8"
          on:click|stopPropagation
        >
          <h2 class="text-4xl font-bold mb-8">{selectedSeries.name}</h2>
          <div
            class="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-6 lg:grid-cols-8 gap-6"
          >
            {#each selectedSeries.comics as comic}
              <a href="/comic/{comic.id}" class="block group">
                <img
                  src={`http://localhost:8000/comics/${comic.id}/page/0`}
                  class="w-[100px] h-[100px] object-cover mx-auto rounded-lg shadow-lg group-hover:shadow-blue-500 transition"
                />
